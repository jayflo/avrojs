...
var PUBLIC_VALIDATIONS_MAP = collections.PUBLIC_VALIDATIONS_MAP;
...
module.exports = Object.freeze({
  ...
  getKeyNamePriorityInfo: getKeyNamePriorityInfo
});
...
ValidationManager.applyValidation = function(v, value) {
  return v.validator.apply(null, v.args.concat(value));
};
...
function finalize() {
  this._validations.push(this.$avro, this.$children);
  this._validations.sort(function(v1, v2) {
    return v1.priority - v2.priority;
  });
  this._validations.forEach(Object.freeze);
  Object.freeze(this._validations);
  Object.freeze(this);
}
...
function getPublicValidation(registry, namespace, name, priority, value) {
  var entry = registry(namespace, name) ||
              PUBLIC_VALIDATIONS_MAP[unqualifiedName(name)];

  if (!entry)
    throw new Error('Unknown validator %s', name);
